 ...
 * <br/>File Name: AnchorDx_DataOrganizer.java
 * <br/>Date: 2017-03-30
...
public class AnchorDx_DataOrganizer
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		for (int len = 0; len < args_len; len+=2) {
			if (args[len].equals("-O") || args[len].equals("-o")) {
				...
				System.out.println("-M or -m\t Set work pattern. Inuput 0 or 1, 0 representative operate all data and 1 operate update data. The default value is 0.");
	...
	/**
	 * 远程复制文件的方法：
	 * 利用rsync远程复制zhirong_lu@192.192.192.200:/wdmycloud/anchordx_cloud/杨莹莹/基准所有项目收样信息表样本处理追踪表目录到本地，程序正常返回0，否则返回-1.
	 */
					...
					for (File porject_name : pathname.listFiles())
					{
						...
						if (porject_name.isFile()) {
							...
							if( Remove_suffix.contains(excel_part_name) && !Remove_suffix.contains("~$") ){
								...
								String Output_File = dir_path_name+"/"+Remove_suffix+"_"+Date+"_All_"+".txt"; // 输出目标的文件名
								...
								// 做软链接，链接总数据文件
								try {
									String Link_File_All =  target_Path + dir_name +"/"+Remove_suffix+"_All_"+".txt"; // 带路径的链接文件名
					...
					for (File porject_name : pathname.listFiles())
					{
						...
						if (porject_name.isFile()) {
							...
							if(Remove_suffix.contains(excel_part_name)  && !Remove_suffix.contains("~$")){
								...
								String Source_File = porject_name.getParent() + "/"+ porject_name.getName(); // 带路径的源文件名
								String Output_File = dir_path_name+"/"+Remove_suffix+"_"+Date+"_All_"+".txt"; // 输出目标的文件名
								String LastTime_Output_File = target_Path + dir_name +"/"+Remove_suffix+"_All_"+".txt"; // 上次输出总数据的文件
								String Updata_File = dir_path_name+"/"+Remove_suffix+"_"+Date+"_Updata_"+".txt";// 更新数据的文件 
								String Source_All_Output_File = "Master/"+Remove_suffix+"_"+Date+"_All_"+".txt"; // 链接的总数据文件的源文件
								String Source_Updata_Output_File = "Master/"+Remove_suffix+"_"+Date+"_Updata_"+".txt"; // 链接的更新数据文件的源文件
								...
								try {
									...
									BufferedReader input_All = new BufferedReader(new InputStreamReader(process_All.getInputStream()));
									...
									String Link_File_Updata =  target_Path + dir_name +"/"+Remove_suffix+"_Updata_"+".txt"; // 带路径的链接文件名
									String cmd_Updata = "ln -s -f " + Source_Updata_Output_File + " " + Link_File_Updata; // Linux命令
									Process process_Updata = Runtime.getRuntime().exec(cmd_Updata); // 调用Linux命令，链接更新数据文件
									BufferedReader input_Updata = new BufferedReader(new InputStreamReader(process_Updata.getInputStream()));
